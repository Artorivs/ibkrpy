...
class TradingDashboard:
    def __init__(self, root):
        ...
        self.style.configure(
            "Header.TLabel", font=("Helvetica", 14, "bold"), foreground="#00E5FF"
        )
        self.style.configure(
            "Treeview",
            background="#2D2D2D",
    ...
    def _build_ui(self):
        ...
        title_lbl = ttk.Label(
            top_frame, text="🟢 系統狀態: 實盤/模擬盤自動運行中", style="Header.TLabel"
        )
        ...
        self.account_info_text = tk.Text(
            right_frame,
            width=50,
            height=10,
            bg="#2D2D2D",
        ...
        self.chart_selector = ttk.Combobox(
            chart_ctrl_frame, state="readonly", width=20, font=("Helvetica", 11)
        )
        ...
        ttk.Label(
            bottom_frame,
            text="📡 底層監控終端 (API 防護 / MAD 剔除 / 宏觀警報)",
            style="Header.TLabel",
        ).pack(anchor=tk.W, pady=(0, 5))
    ...
    def _update_data(self):
        ...
        try:
            ...
            if account_data:
                ...
                if position_data:
                    for p in position_data:
                        ...
                        new_acc_text += (
                            f"  ➤ {p['symbol']:<6}: {color_prefix}{p['position']} 股\n"
                        )
                else:
            ...
            available_symbols = [
                f"📊 標的走勢: {row['symbol']}" for row in symbols_data
            ]
            ...
            if selection == "💰 帳戶收益曲線 (Equity)":
                df_eq = pd.DataFrame(
                    self.db._fetch_sync(
                        "SELECT timestamp, net_liquidation FROM equity_history ORDER BY timestamp DESC LIMIT 300"
                    )
                )
                if not df_eq.empty:
                ...
                df_market = pd.DataFrame(
                    self.db._fetch_sync(
                        "SELECT timestamp, close FROM market_data WHERE symbol = ? AND timeframe = '1 day' ORDER BY timestamp DESC LIMIT 300",
                        (sym,),
                    )
                )
                if not df_market.empty:
                    df_market["timestamp"] = pd.to_datetime(
                        df_market["timestamp"], format="mixed", utc=True
                    )
                    ...
                    df_market["Return_Pct"] = (
                        (df_market["close"] / first_close) - 1
                    ) * 100
                    ...
                    self.ax2.legend(
                        loc="lower right", facecolor="#1E1E1E", labelcolor="white"
                    )

                    self.ax.set_title(
                        f"{sym} - Price Trend & Momentum Potential", color="#FFFFFF"
                    )
