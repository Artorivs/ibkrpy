...
class IBKRDataManager:
    ...
    async def _rate_limit_historical(self):
        ...
        async with self._hist_req_lock:
            ...
            if len(self._hist_req_timestamps) >= 55:
                ...
                logger.warning(f"🛡️ [API 防護] 歷史數據請求逼近券商上限，系統自動進入降溫休眠 {sleep_time:.1f} 秒...")
    ...
    async def fetch_historical_data(
        ...
        if contract.conId:
            query_contract = Contract(
                ...
                secType=contract.secType or 'STK', 
        ...
        try:
            ...
            logger.warning(f"獲取 {contract.symbol if contract.symbol else contract.conId} 歷史數據失敗: {e}")
            return pd.DataFrame()
    ...
    async def subscribe_realtime_bars(
        ...
        if len(self._active_subscriptions) >= self.max_subscriptions:
            logger.warning(f"❌ [API 防護] 拒絕訂閱 {contract.symbol}：已達實時訂閱安全上限 ({self.max_subscriptions} 檔)。")
            return
        ...
        try:
            ...
            logger.debug(f"成功訂閱 {contract.symbol} 的實時數據 (目前水位: {len(self._active_subscriptions)}/{self.max_subscriptions})。")
        except Exception as e:
